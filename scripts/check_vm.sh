...
if [ -z "$VMS_DEFAULT_SA" ]; then
  ...
  echo "$VMS_DEFAULT_SA" | while IFS= read -r line; do [ -n "$line" ] && info "$line"; done
fi
echo ""
...
if [ -z "$VMS_FULL_ACCESS" ]; then
  ...
  echo "$VMS_FULL_ACCESS" | while IFS= read -r line; do [ -n "$line" ] && info "$line"; done
fi
echo ""
...
if [ -z "$VMS_NO_BLOCK" ]; then
  ...
  echo "$VMS_NO_BLOCK" | while IFS= read -r line; do [ -n "$line" ] && info "$line"; done
fi
echo ""
...
if [ -z "$VMS_NO_OSLOGIN" ]; then
  ...
  echo "$VMS_NO_OSLOGIN" | while IFS= read -r line; do [ -n "$line" ] && info "$line"; done
fi
echo ""
...
if [ -z "$VMS_SERIAL" ]; then
  ...
  echo "$VMS_SERIAL" | while IFS= read -r line; do [ -n "$line" ] && info "$line"; done
fi
echo ""
